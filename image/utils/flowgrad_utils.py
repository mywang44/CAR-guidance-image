...
class clip_semantic_loss():
    def __init__(self, text, img, device, alpha=0.5, replicate=20, inverse_scaler=None):
        ...
        self.verbose = False  # 控制是否打印 regu/reward

    def L_N(self, x):
        # 【修改1】必须用 x 的 shape，因为此时输入的是 4 条轨迹 (B=4)，而 self.img 只有 1 张
        batch_size = x.shape[0] 
        
        # 【修改2】将每张图的像素展平后，单独求该图的平均误差。保留 Batch 维度，变成形状为 [B] 的向量
...
def flowgrad_optimization(z0, u_ind, dynamic, generate_traj, N=100, L_N=None, u_init=None,  number_of_iterations=10, straightness_threshold=1e-3, lr=1.0):
    ...
    for i in range(number_of_iterations):
        ...
        inputs.requires_grad = True
        loss = L_N(inputs)
        lam = torch.autograd.grad(loss, inputs)[0] # 伴随变量 (Adjoint Variable) $\lambda_t = \nabla_{x_t}\mathcal{J}$
        ...
        for j in range(N-1, -1, -1):
            ...
            # 为什么要用 VJP 而不是直接 .backward()？ 如果你直接对整个 100 步的轨迹做反向传播，显存会爆炸。通过手动调用 vjp，算法可以一步一步地把梯度传回来，每算完一步就把中间变量删掉。这极大地节省了显存，使得在普通显卡上优化高分辨率图像成为可能。
