...
class ImageGCovAGMOnlineGuidance:
    ...
    def _compute_online_loss_gradient(self, xs_stacked, ts_stacked, vs_stacked,
        ...
        with torch.enable_grad():
            ...
            # =======================================================
            # 🔴 target 归一化：把 teacher 梯度归一化成单位向量，专攻方向。
            # 大尺度（norm 量级差异）会分散网络注意力，归一化后让网络
            # 只学方向。推理侧 compute_guidance 用 lr_res 乘回放大。
            # 注意：归一化后 ratio 指标作废（target 是单位向量，pred 也学
            # 单位方向，ratio 自然趋近 1），只盯 cosine。
            # =======================================================
            grad_energy = grad_energy / (clip_norm + 1e-8)
    ...
    def compute_guidance(self, x, t, v_uncond, need_higher_order=False):
        ...
        # 🔴 配套 target 归一化：训练时 target 是单位向量，网络学到的是单位方向，
        # 这里乘回 lr_res 把方向放大成真正尺度的力。
        lr_res = getattr(self.cfg, "lr_res", 30.0)
        g_res = lr_res * self.learned_guidance_model(x_req, t_norm)   # [B, C, H, W]，已是向量场
